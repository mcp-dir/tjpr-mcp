...
### TJPR case law for Claude, Cursor and AI agents

Search case law from the Paraná state court (TJPR) straight from Claude, ChatGPT or your own agent, covering cível, consumidor, família e imobiliário. Every ruling comes with the judging body, rapporteur, date, the exact snippet that matched your query and the link to the official site. The same connection reaches 16 other Brazilian courts. Free, no login, hosted by the platform.

- ⚖️ **TJPR** plus 16 other Brazilian courts on the same connection
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Jurisprudência TJPR` and **URL** `https://api.mcp.ai/p_tjpr`.

### Cursor

[➕ Install Jurisprudência TJPR in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=tjpr&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF90anByIn0=)

### VS Code (Copilot Chat)

[➕ Install Jurisprudência TJPR in VS Code](vscode:mcp/install?name=tjpr&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_tjpr%22%7D)
...
```
Recent TJPR rulings on consumer damages
Compare TJPR and the Superior Court on limitation periods
...
**O TJPR é o único tribunal?**
Não. A conexão serve 17 tribunais: além do TJPR, STF, STJ, TST, TRF3, TRF4, CARF e os demais tribunais de justiça estaduais. Restringir a busca ao TJPR é opcional, e comparar com as cortes superiores é uma pergunta só.
...
- 📧 [tjpr@mcp.ai](mailto:tjpr@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/tjpr-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_tjpr` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
